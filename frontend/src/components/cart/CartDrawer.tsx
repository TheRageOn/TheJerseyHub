...
export default function CartDrawer() {
  const { items, totalItems, subtotal, isCartOpen, closeCart, updateQuantity, removeFromCart } =
    useCart();
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 right-0 bottom-0 z-50 w-full max-w-md backdrop-blur-2xl flex flex-col transition-all duration-300 shadow-2xl animate-in slide-in-from-right duration-300 ${
          isWhite
            ...
            items.map((item) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 font-mono text-[10px] opacity-60">
                    <span>SIZE: [{item.size}]</span>
                    <span>•</span>
                    <span className="text-[#ff5500] font-semibold">${item.priceNumeric.toFixed(2)}</span>
                  </div>
        ...
        {items.length > 0 && (
          ...
          >
            ...
            <button
              ...
              className="w-full h-12 bg-gradient-to-r from-[#ff5500] to-[#e64000] hover:from-[#ff6614] hover:to-[#f04800] text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all duration-300 cursor-pointer shadow-[0_6px_20px_rgba(255,85,0,0.35)] flex items-center justify-center gap-2"
            >
