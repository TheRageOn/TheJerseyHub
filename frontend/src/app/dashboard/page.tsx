...
export default function DashboardPage() {
  ...
  const [adminJerseys, setAdminJerseys] = useState<DBJersey[]>(FALLBACK_CATALOG);
  ...
  const [preferredSize, setPreferredSize] = useState<"S" | "M" | "L" | "XL" | "XXL">("L");
  ...
  const handleUpdateOrderStatus = async (orderId: string, newStatus: string) => {
    ...
    if (res.success) {
      setOrders((prev) =>
        prev.map((ord) => (ord._id === orderId || ord.id === orderId ? { ...ord, orderStatus: newStatus as DBOrder["orderStatus"] } : ord))
      );
      toast.success(`Order ${orderId.slice(-6).toUpperCase()} status updated to ${newStatus.toUpperCase()}`);
    } else {
  ...
  const handlePlacementToggle = async (
    ...
    setAdminJerseys((prev) =>
      prev.map((k) =>
        (k._id === kitId || k.id === kitId || k.code === kitId) ? { ...k, [field]: newVal } : k
      )
    );

    const res = await productService.updatePlacement(kitId, { [field]: newVal });
    if (res.success) {
      toast.success(`${field.toUpperCase()} synced to MongoDB: ${newVal ? "ON" : "OFF"}`);
    } else {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    const kitKey = deleteModalKit._id || deleteModalKit.id || deleteModalKit.code;
    const kitCode = deleteModalKit.code;
    const res = await productService.deleteJersey(kitKey);
    setIsDeleting(false);

    if (res.success) {
      setAdminJerseys((prev) => prev.filter((k) => (k._id || k.id || k.code) !== kitKey));
  ...
  const handleSaveKitForm = async (e: React.FormEvent) => {
    ...
    if (editingJersey) {
      const kitKey = editingJersey._id || editingJersey.id || editingJersey.code;
      const res = await productService.updateJersey(kitKey, kitForm);
      if (res.success) {
        toast.success(`[${kitForm.code}] updated successfully in MongoDB Atlas!`);
  ...
  const handlePreviewMouseLeave = () => {
    if (!previewRef.current) return;
    previewRef.current.style.transform = "perspective(800px) rotateY(0deg) rotateX(0deg) scale(1)";
    previewRef.current.style.filter = "drop-shadow(0 15px 25px rgba(0,0,0,0.3))";
  };
          ...
          {isAdmin && !adminCollectorPreview ? (
            <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-200">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                ...
                >
                  <span className="text-[9px] sm:text-[10px] opacity-50 uppercase block truncate">3D LANDING ORBIT</span>
                  <p className="text-base sm:text-xl font-bold text-[#ff5500] mt-0.5 sm:mt-1 truncate">
                    {landingKitsCount} ACTIVE KITS
                  </p>
                  <span className="text-[9px] text-emerald-500 block">Rendering on Home</span>
                </div>
                ...
                >
                  <span className="text-[9px] sm:text-[10px] opacity-50 uppercase block truncate">SHOP MARKETPLACE</span>
                  <p className="text-base sm:text-xl font-bold text-[#ff5500] mt-0.5 sm:mt-1 truncate">
                    {shopKitsCount} ARCHIVED
                  </p>
                  <span className="text-[9px] text-emerald-500 block">Available in /shop</span>
                </div>
                ...
                >
                  <span className="text-[9px] sm:text-[10px] opacity-50 uppercase block truncate">TOTAL CATALOG</span>
                  <p className="text-base sm:text-xl font-bold text-[#ff5500] mt-0.5 sm:mt-1 truncate">
                    {adminJerseys.length} EDITIONS
                  </p>
                  <span className="text-[9px] text-blue-500 block">MongoDB Atlas</span>
                </div>
                ...
                >
                  <span className="text-[9px] sm:text-[10px] opacity-50 uppercase block truncate">VERCEL PRODUCTION</span>
                  <p className="text-base sm:text-xl font-bold text-emerald-500 mt-0.5 sm:mt-1 truncate">
                    CONNECTED
                  </p>
                  <span className="text-[9px] opacity-60 block">Isomorphic Fallback</span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 sm:gap-2 border-b pb-3 border-black/10 dark:border-white/10 font-mono text-[10.5px] sm:text-xs">
                {[
                  { id: "admin_products" as const, label: "3D ORBIT & PLACEMENT" },
                  { id: "admin_inventory" as const, label: "INVENTORY & STOCK" },
                  { id: "admin_orders" as const, label: `CUSTOMER DISPATCH (${orders.length})` },
                  { id: "admin_users" as const, label: `COLLECTOR DIRECTORY (${usersList.length})` },
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-4 border-black/10 dark:border-white/10 font-mono text-xs">
                    <div>
                      ...
                      <p className="text-[11px] opacity-60 mt-0.5">
                        Add, edit, remove, and toggle live rendering on the 3D Landing Page Carousel and Shop Archive.
                      </p>
                    </div>
                      ...
                      <table className="w-full text-left font-mono text-xs min-w-[780px]">
                        <thead>
                          <tr className="border-b text-[10px] opacity-50 uppercase border-black/10 dark:border-white/10">
                            ...
                            <th className="pb-3 px-2 text-center">3D LANDING</th>
                            <th className="pb-3 px-2 text-center">ORBIT ORDER</th>
                            <th className="pb-3 px-2 text-center">SHOP ARCHIVE</th>
                        ...
                        <tbody className="divide-y divide-black/5 dark:divide-white/5">
                          {displayedAdminJerseys.map((kit) => {
                            ...
                            return (
                              <tr key={kitId} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                                <td className="py-3 px-2">
                                  <div className="flex items-center gap-2.5">
                                    ...
                                    <span className="font-bold text-[#ff5500]">{kit.code}</span>
                                  </div>
                                </td>
                                <td className="py-3 px-2">
                                  <p className="font-bold truncate max-w-[200px]">{kit.name}</p>
                                  <p className="text-[10px] opacity-50">{kit.club} • {kit.category.toUpperCase()}</p>
                                </td>
                                <td className="py-3 px-2 font-bold">{kit.price}</td>
                                
                                {/* 3D Landing Carousel Toggle */}
                                <td className="py-3 px-2 text-center">
                                  <button
                                    onClick={() => handlePlacementToggle(kitId, "showOnLanding", kit.showOnLanding)}
                                ...
                                <td className="py-3 px-2 text-center">
                                  <select
                                    ...
                                    onChange={(e) => handleOrderChange(kitId, Number(e.target.value))}
                                    className={`py-1 px-2 rounded-lg text-xs font-mono border outline-none disabled:opacity-30 cursor-pointer ${
                                      isWhite ? "bg-[#faf7f0] border-black/15" : "bg-[#16161a] border-white/15"
                                    }`}
                                  >
                                    {Array.from({ length: Math.max(adminJerseys.length, 12) }, (_, i) => (
                                ...
                                <td className="py-3 px-2 text-center">
                                  <button
                                    onClick={() => handlePlacementToggle(kitId, "showInShop", kit.showInShop)}
                                    className={`px-3 py-1.5 rounded-xl font-mono text-[10px] font-bold border transition-all cursor-pointer ${
                                      kit.showInShop
              ...
              {activeTab === "admin_inventory" && (
                ...
                >
                  <div className="flex items-center justify-between border-b pb-4 border-black/10 dark:border-white/10 font-mono text-xs">
                    <div>
                      <h3 className="font-bold text-sm sm:text-base">[INVENTORY ALLOCATION & SIZES]</h3>
                      <p className="text-[11px] opacity-60 mt-0.5">Real-time stock units and size matrix across catalog.</p>
                    </div>
                    <span className="text-[#ff5500] font-bold">{adminJerseys.length} TOTAL ITEMS</span>
                  </div>
                    ...
                    {adminJerseys.map((kit) => (
                      <div
                        ...
                        className={`p-4 rounded-2xl border font-mono text-xs space-y-2.5 ${
                          isWhite ? "bg-[#faf7f0] border-black/10" : "bg-[#141418] border-white/10"
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`font-bold ${kit.stock && kit.stock < 10 ? "text-amber-500" : "text-emerald-500"}`}>
                        ...
                        <div className="flex items-center gap-1 text-[10px] opacity-70">
                          <span>SIZES:</span>
                          {(kit.sizesAvailable || ["S", "M", "L", "XL"]).map((sz) => (
                            <span key={sz} className="px-1.5 py-0.5 rounded border border-black/10 dark:border-white/10 font-bold">
                              {sz}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-black/5 dark:border-white/5">
                          <span className="font-bold text-[#ff5500]">{kit.price}</span>
                          <button
                            onClick={() => handleStartEdit(kit)}
              ...
              {activeTab === "admin_orders" && (
                ...
                >
                  <div className="flex items-center justify-between font-mono text-xs border-b pb-3 border-black/10 dark:border-white/10">
                    <span className="font-bold tracking-wider">[CUSTOMER DISPATCH & FULFILLMENT]</span>
                    <span className="opacity-50">{orders.length} ACTIVE ORDERS</span>
                  </div>
                      ...
                      <table className="w-full text-left font-mono text-xs min-w-[540px]">
                        ...
                        <tbody className="divide-y divide-black/5 dark:divide-white/5">
                          {orders.map((order) => {
                            ...
                            const customerObj = typeof order.user === "object" ? order.user : null;
                            const customerName = customerObj?.name || order.shippingAddress?.fullName || "Collector";
                            const customerEmail = customerObj?.email || "verified@user";
                            const itemSpec = order.items?.[0] ? `${order.items[0].name} (${order.items[0].size})` : "Match Spec Kit";

                            return (
                              <tr key={orderId} className="hover:bg-black/5 dark:hover:bg-white/5">
                                <td className="py-3 px-2 font-bold text-[#ff5500]">#{orderId.slice(-6).toUpperCase()}</td>
                                <td className="py-3 px-2">
                                  <p className="font-bold">{customerName}</p>
                                  <p className="text-[9.5px] opacity-50 truncate max-w-[140px]">{customerEmail}</p>
                                </td>
                                <td className="py-3 px-2 text-[11px] truncate max-w-[180px]">{itemSpec}</td>
              ...
              {activeTab === "admin_users" && (
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-3 border-black/10 dark:border-white/10 font-mono text-xs">
                    <div>
                      <span className="font-bold">[COLLECTORS DIRECTORY & PERMISSIONS]</span>
              ...
              {activeTab === "admin_settings" && (
                ...
                >
                  <div className="border-b pb-4 border-black/10 dark:border-white/10">
                    <h3 className="font-mono text-sm sm:text-base font-bold">[STORE ENGINE CONFIGURATION]</h3>
                    <p className="text-xs opacity-60 mt-0.5">
                      Configure store parameters, global announcements, and dispatch rules.
                    </p>
                  </div>

                  <div className="space-y-4 max-w-xl font-mono text-xs">
                    <div>
                      <label className="block text-[10px] uppercase opacity-60 mb-1">Global Announcement Banner</label>
                      <input
                        type="text"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[10px] uppercase opacity-60 mb-1">Currency Code</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-[10px] uppercase opacity-60 mb-1">Shipping Policy</label>
                        <input
                          type="text"
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        toast.success("Store engine parameters saved successfully!");
                      }}
            ...
            <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-200">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 border-b pb-4 border-black/10 dark:border-white/10">
                <div>
                  ...
                  <p className="text-[10px] sm:text-[11px] font-mono opacity-50 mt-0.5 truncate">
                    COLLECTOR ID: {user?.id?.slice(-8).toUpperCase() || "TJH-COL-8902"} • VERIFIED MEMBER
                  </p>
                </div>
                ...
                <div className="space-y-6 sm:space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
                        ...
                        >
                          ...
                          <p className="font-mono text-[9.5px] sm:text-[10px] font-bold">AUTH ALERTS</p>
                          <p className="font-mono text-[8px] sm:text-[8.5px] opacity-50 uppercase">MATCH VERIFY</p>
                        </div>
                        ...
                        >
                          ...
                          <p className="font-mono text-[9.5px] sm:text-[10px] font-bold">RESTOCK</p>
                          <p className="font-mono text-[8px] sm:text-[8.5px] opacity-50 uppercase">WISHLIST</p>
                        </div>
                        ...
                        >
                          ...
                          <p className="font-mono text-[9.5px] sm:text-[10px] font-bold">RESERVE</p>
                          <p className="font-mono text-[8px] sm:text-[8.5px] opacity-50 uppercase">LIVE DROPS</p>
                        </div>
                        ...
                        >
                          ...
                          <p className="font-mono text-[9.5px] sm:text-[10px] font-bold">COD PAY</p>
                          <p className="font-mono text-[8px] sm:text-[8.5px] opacity-50 uppercase">DELIVERY</p>
                        </div>
                      </div>
                      ...
                      {orders.length > 0 ? (
                        ...
                        >
                          ...
                          <p className="font-bold text-xs">ORDER #{orders[0]._id.slice(-6).toUpperCase()}</p>
                          <p className="text-[10px] opacity-60 mt-0.5">
                            {orders[0].items?.[0]?.name || "Match Spec Kit"} • ${orders[0].totalAmount}
                          </p>
                        </div>
                      ) : (
                        <div className={`p-4 rounded-2xl border text-center font-mono text-[10.5px] opacity-60 ${
                    ...
                    <div className="space-y-4">
                      ...
                      >
                        ...
                        <p className="text-[10px] opacity-50 truncate">ALLOCATION : 25 UNITS // SHARP</p>
                      </div>
                    </div>
                    ...
                    >
                      <div className="flex items-center justify-between font-mono text-xs">
                        <span className="opacity-60 truncate">FEATURED SPEC: FC BARCELONA</span>
                        <span className="font-bold text-[#ff5500] shrink-0">$125.00</span>
                      </div>
                  ...
                  <section className="space-y-4">
                    <div className="flex items-center justify-between font-mono text-xs border-b pb-2 border-black/10 dark:border-white/10">
                      <span className="font-bold tracking-wider">[VAULT COLLECTION // ACTIVE SPECIFICATIONS]</span>
                      <Link href="/shop" className="text-[#ff5500] hover:underline">
                        + Discover More
                      </Link>
                    </div>
                      ...
                      {adminJerseys.slice(0, 4).map((kit) => (
                        <div
                          ...
                          className={`p-3 sm:p-4 rounded-2xl border font-mono text-xs flex flex-col justify-between relative transition-all ${
                            isWhite ? "bg-white border-black/10 shadow-sm" : "bg-[#14141a] border-white/10"
                          }`}
                        >
                          ...
                          <div className="mt-2 space-y-1">
                            ...
                            <p className="text-[9px] opacity-50 truncate">{kit.name}</p>

                            <div className="flex items-center justify-between pt-2 border-t border-black/5 dark:border-white/5">
                              <span className="text-[#ff5500] font-bold text-xs">{kit.price}</span>
                              <Link
                                href="/shop"
              ...
              {activeTab === "orders" && (
                ...
                >
                  <div className="flex items-center justify-between font-mono text-xs border-b pb-3 border-black/10 dark:border-white/10">
                    <span className="font-bold tracking-wider">[PERSONAL DISPATCH & ORDER ARCHIVE]</span>
                      ...
                      <table className="w-full text-left font-mono text-xs min-w-[500px]">
                        ...
                        <tbody className="divide-y divide-black/5 dark:divide-white/5">
                          {orders.map((row) => (
                            ...
                            >
                              <td className="p-3 sm:p-3.5 font-bold text-[#ff5500]">#{row._id.slice(-6).toUpperCase()}</td>
                              ...
                              <td className="p-3 sm:p-3.5 font-bold">${row.totalAmount}</td>
              ...
              {activeTab === "settings" && (
                <div className="space-y-8">
                  ...
                  >
                    <div className="border-b pb-4 border-black/10 dark:border-white/10">
                      <h3 className="font-mono text-sm sm:text-base font-bold">[COLLECTOR IDENTITY & SIZING]</h3>
                      <p className="text-xs opacity-60 mt-0.5">
                        Manage your account credentials, size preferences, and club affiliation.
                      </p>
                    </div>

                    <form onSubmit={handleSaveProfile} className="space-y-4 max-w-2xl font-mono text-xs">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] uppercase opacity-60 mb-1">Full Name</label>
                          <input
                            type="text"
                            value={profileName}
                        ...
                        <div>
                          <label className="block text-[10px] uppercase opacity-60 mb-1">Email Address (Read-only)</label>
                          <input
                            type="email"
                            disabled
                            value={profileEmail}
                        ...
                        <div>
                          <label className="block text-[10px] uppercase opacity-60 mb-1">Contact Phone</label>
                          <input
                            type="tel"
                            value={profilePhone}
                        ...
                        <div>
                          <label className="block text-[10px] uppercase opacity-60 mb-1">Favorite Club</label>
                          <input
                            type="text"
                            value={favoriteClub}
                      ...
                      <div>
                        <label className="block text-[10px] uppercase opacity-60 mb-2">Preferred Jersey Fit / Size</label>
                  ...
                  >
                    <div className="border-b pb-4 border-black/10 dark:border-white/10">
                      <h3 className="font-mono text-sm sm:text-base font-bold">[SHIPPING & DISPATCH COORDINATES]</h3>
                      <p className="text-xs opacity-60 mt-0.5">
                        Default address used for instant Cash on Delivery (COD) dispatches.
                      </p>
                    </div>

                    <div className="space-y-4 max-w-2xl font-mono text-xs">
                      <div>
                        <label className="block text-[10px] uppercase opacity-60 mb-1">Street Address</label>
                        <input
                          type="text"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div>
                          <label className="block text-[10px] uppercase opacity-60 mb-1">City</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="block text-[10px] uppercase opacity-60 mb-1">Postal Code</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="block text-[10px] uppercase opacity-60 mb-1">Country</label>
                          <input
                            type="text"
                      ...
                      <div>
                        <label className="block text-[10px] uppercase opacity-60 mb-1">Courier Delivery Instructions</label>
                        <input
                          type="text"
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          toast.success("Delivery coordinates updated successfully!");
                        }}
                        className="px-6 py-3 bg-[#ff5500] text-white font-bold rounded-xl shadow-[0_4px_15px_rgba(255,85,0,0.35)] cursor-pointer"
                      >
                  ...
                  >
                    <div className="border-b pb-4 border-black/10 dark:border-white/10">
                      <h3 className="font-mono text-sm sm:text-base font-bold">[SECURITY & ENCRYPTED CREDENTIALS]</h3>
                      <p className="text-xs opacity-60 mt-0.5">
                    ...
                    <form onSubmit={handlePasswordChange} className="space-y-4 max-w-xl font-mono text-xs">
                      <div>
                        <label className="block text-[10px] uppercase opacity-60 mb-1">Current Password</label>
                        <input
                          type="password"
                          required
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <label className="block text-[10px] uppercase opacity-60 mb-1">New Password</label>
                          <input
                            type="password"
                            required
                            value={newPassword}
                        ...
                        <div>
                          <label className="block text-[10px] uppercase opacity-60 mb-1">Confirm New Password</label>
                          <input
                            type="password"
                            required
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {[
                          ...
                          { name: "Real Madrid", src: "/images/real-jersey.svg" },
                          { name: "Arsenal Cannon", src: "/images/arsenal-jersey.svg" },
                          { name: "Man United Devil", src: "/images/manchester-united-jersey.svg" },
                        ].map((preset) => (
                          <button
                            key={preset.src}
                            type="button"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setKitForm({ ...kitForm, showOnLanding: e.target.checked })}
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setKitForm({ ...kitForm, showInShop: e.target.checked })}
                      className="w-4 h-4 accent-[#ff5500] rounded"
                    />
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setKitForm({ ...kitForm, featured: e.target.checked })}
                      className="w-4 h-4 accent-[#ff5500] rounded"
                    />
          ...
          {isAdmin ? (
            <>
              <button
                ...
                className={`py-1.5 px-3 rounded-xl font-mono text-[10px] font-bold flex items-center gap-1.5 cursor-pointer ${
                  activeTab === "admin_products" ? "bg-[#ff5500] text-white" : "opacity-60"
                }`}
              >
                <span>3D ORBIT</span>
              </button>
              <button
                ...
                className={`py-1.5 px-3 rounded-xl font-mono text-[10px] font-bold flex items-center gap-1.5 cursor-pointer ${
                  activeTab === "admin_inventory" ? "bg-[#ff5500] text-white" : "opacity-60"
                }`}
              >
                <span>STOCK</span>
              </button>
              <button
                ...
                className={`py-1.5 px-3 rounded-xl font-mono text-[10px] font-bold flex items-center gap-1.5 cursor-pointer ${
                  activeTab === "admin_orders" ? "bg-[#ff5500] text-white" : "opacity-60"
                }`}
              >
                <span>DISPATCH</span>
              </button>
            ...
            <>
              <button
                onClick={() => setActiveTab("vault")}
                className={`py-1.5 px-3 rounded-xl font-mono text-[10px] font-bold flex items-center gap-1.5 cursor-pointer ${
                  activeTab === "vault" ? "bg-[#ff5500] text-white" : "opacity-60"
                }`}
              >
                <span>VAULT</span>
              </button>
              <button
                onClick={() => setActiveTab("settings")}
                className={`py-1.5 px-3 rounded-xl font-mono text-[10px] font-bold flex items-center gap-1.5 cursor-pointer ${
                  activeTab === "settings" ? "bg-[#ff5500] text-white" : "opacity-60"
                }`}
              >
                <span>SETTINGS</span>
              </button>
